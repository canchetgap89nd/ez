...
class UpdateProductRequest extends FormRequest
{
    ...
    public function rules()
    {
        $rules = [
            ...
            'product.price' => 'required|numeric|min:0|max:99000000000',
            'product.quantity' => 'nullable|numeric|min:0|max:99000000',
            'product.thumb' => 'nullable|max:500',
            'childs.*' => 'nullable',
            'childs.*.props' => 'required|array',
            'childs.*.imagesNew' => 'nullable|array',
            'childs.*.imagesDel' => 'nullable|array',
            'childs.*.prod_thumb' => 'nullable|max:500',
        ...
        $childs = Request::input('childs.*.id');
        $ruleCodeParent =   [
                                'regex:/^[a-zA-Z0-9]*$/',
        ...
        foreach ($childs as $k1 => $value1) {
            ...
            $ruleCodeChild =    [
                                    ...
                                    'max:10',
                                    'nullable',
                                    Rule::unique('products', 'prod_code')->ignore($value1)->where(function ($query) {
                                        return $query->where('user_id', Auth::id());
                                    })
                                ];
            foreach ($childs as $k2 => $value2) {
                if ($k1 !== $k2 && $value2 != null && $value1 !== null) {
