...
class ApiFacebookController extends Controller
{
    ...
    public function getJustMessage($conversationId, $access_token, $timeMess)
    {
        ...
        try {
            // Returns a `Facebook\FacebookResponse` object
            $response = $fb->get('/'.$conversationId.'/messages?fields=from,message,attachments,created_time,to&limit=10', $access_token);
        } catch(\Facebook\Exceptions\FacebookResponseException $e) {
            \Log::info(print_r('Graph returned an error: ' . $e->getMessage(), true));
            return null;
        } catch(\Facebook\Exceptions\FacebookSDKException $e) {
    ...
    public function getInfoMessage($messageId, $access_token)
    {
        ...
        try {
            // Returns a `Facebook\FacebookResponse` object
            $response = $fb->get('/'.$messageId.'?fields=from,message,attachments,created_time,to', $access_token);
        } catch(\Facebook\Exceptions\FacebookResponseException $e) {
            \Log::info(print_r('Graph returned an error: ' . $e->getMessage(), true));
            return [];
        } catch(\Facebook\Exceptions\FacebookSDKException $e) {
